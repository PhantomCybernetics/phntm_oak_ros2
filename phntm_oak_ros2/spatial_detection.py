...
async def async_loop():
    ...
    try:
        ...
        with dai.Device(pipeline) as device:
            ...
            previewQueue = device.getOutputQueue(name="rgb", maxSize=10, blocking=False)
            detectionNNQueue = device.getOutputQueue(name="detections", maxSize=10, blocking=False)
            depthQueue = device.getOutputQueue(name="depth", maxSize=10, blocking=False)
            networkQueue = device.getOutputQueue(name="nnNetwork", maxSize=10, blocking=False)
            
            rgbH264Queue = device.getOutputQueue(name="rgbH264", maxSize=10, blocking=False)
